...
#pragma config(Servo,	 srvo_S2_C1_1,		S_score,							tServoStandard)
#pragma config(Servo,	 srvo_S2_C1_2,		S_flag_out,						tServoStandard)
...
void InitializeRobot (){
	nMotorEncoder[M_slide_R] = 0;
	nMotorEncoder[M_slide_L] = 0;
	servo[S_flag_out] = 235; //put flaggerin correct position

	// TODO: initialize score servo to correct location
...
task main() {
	...
	while(true) {
		...
		if (FLAGGER_OUT_BTN) {
			servo[S_flag_out] = 115;
		}
		if (FLAGGER_IN_BTN) {
			servo[S_flag_out] = 235;

		}
		if (SPIN_FLAGGER_CW_BTN) {
			motor[M_flag_spin] = 100;
