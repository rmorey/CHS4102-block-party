...
task DriveUpdate() {
	...
	while (true) {
		...
		if (tilted_drive) { //titled drive mode, the flag spinner is onsidered the front of the robot
			pwr_FR = -js_drive_x - js_rotate;
			pwr_FL = js_drive_y + js_rotate;
			pwr_BR = js_drive_y - js_rotate;
			pwr_BL = -js_drive_x + js_rotate;
		}
